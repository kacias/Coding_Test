...
class Solution:
    ...
    def longestPalindrome(self, s: str) -> str:
        ...
        def expand(left: int, right: int) -> str:
            ...
            print("---------expand:{}:{}-----------".format(left,right))

            while left >= 0 and right <= len(s) and s[left] == s[right - 1]:

                print("***********************")
                print("left before:{}".format(left))
                print("right before:{}".format(right))
                print("***********************")
                left -= 1
                right += 1
                print("left after:{}".format(left))
                print("right afeter:{}".format(right))

            #발견된 서브 스트링 반환
            print("result:{}".format(s[left + 1:right - 1]))
            return s[left + 1:right - 1]
